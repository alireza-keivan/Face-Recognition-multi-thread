...
class FaceRecognitionSystem:
    ...
    def __init__(self, config):
        ...
        # self._init_opencv_window() #<-- REMOVED FOR HEADLESS MODE
    ...
    def _load_known_faces(self):
        ...
        if not known_face_encodings_list:
            ...
            known_encodings_np = np.array(known_face_encodings_list, dtype=np.float32)
            if known_encodings_np.ndim == 1:
                self.known_face_encodings = known_encodings_np.reshape(1, -1)
            else:
                self.known_face_encodings = known_encodings_np
    ...
    def _publish_face_image(self, face_name, base64_image_data, face_type, score):
        ...
        if not (self.mqtt_client and self.mqtt_client.is_connected() and base64_image_data):
            return
        ...
        if face_type == "known":
            ...
            topic = self.MQTT_UNKNOWN_FACE_TOPIC
            payload = {
        ...
        while not self.processing_stopped:
            try:
                ...
                for face in faces:
                    ...
                    h, w, _ = frame.shape
                    padding = 100
                    safe_x1 = max(0, x1 - padding)
                    safe_y1 = max(0, y1 - padding)
                    safe_x2 = min(w, x2 + padding)
                    safe_y2 = min(h, y2 + padding)
        ...
        try:
            while True:
                ...
                if frame is None or frame.size == 0:
                    print("Received empty/NONE frame from webcam. Waiting...")
                    time.sleep(1)
                    continue
                ...
                time.sleep(1.0 / self.FPS_LIMIT if self.FPS_LIMIT > 0 else 0.1)
